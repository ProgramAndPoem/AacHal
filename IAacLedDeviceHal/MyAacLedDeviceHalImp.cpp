...
#define MY_MANUFACURE_NAME L"MyCompany"
...
std::string Wcs2Str(const wchar_t* wcs)
{
	...
	size_t len = WideCharToMultiByte(CP_ACP, 0, wcs, (int) wlen, NULL, 0, NULL, NULL);
	char* str = new char[len + 1];
	str[len] = 0;
	WideCharToMultiByte(CP_ACP, 0, wcs, (int) wlen, str, (int) len + 1, NULL, NULL);
	std::string ret = str;
	delete[] str;
	return ret;
}
...
std::wstring Str2Wcs(const char* str)
{
	...
	size_t wlen = MultiByteToWideChar(CP_ACP, 0, str, len, NULL, 0);
...
HRESULT STDMETHODCALLTYPE MyAacLedDevice::GetCapability(BSTR *capability)
{
	...
	// manufacture :
	gs::GsElementNode manufactureNode;
	manufactureNode = device_node->AddElement(L"manufacturer");
	manufactureNode->SetStr(MY_MANUFACURE_NAME);
	...
	// ledcount :
	size_t led_count = doGetLedCount();
...
HRESULT STDMETHODCALLTYPE MyAacLedDevice::SetEffect(ULONG effectId, ULONG *colors, ULONG numberOfColors)
{
	...
	ULONG speed = 0;
	ULONG direction = 0;
	speed = (ULONG) effect->DefaultSpeedLevel;
	direction = (ULONG) effect->DefaultDirectionLevel;
