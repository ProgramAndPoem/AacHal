...
void main()
{
	...
	GUID myGuid;
	CLSIDFromString(L"{05B8E7A3-7D5D-41DE-B6CC-5E6205837FB7}", &myGuid);
	...
	hr = CoCreateInstance(myGuid, NULL, CLSCTX_INPROC_SERVER,
	...
	for (int i = 0; i < devices_vector.size(); ++i)
	{
		...
		{
			if (getFunction.find(effect.id) == getFunction.end())
				continue;
